...
public class MyOpenClassActivity extends BaseActivity {
    ...
    private CommRecyclerViewAdapter<MyOpenClassResponse.mData> mAdapter;
...
//        map.put("user_id", 1);
        ...
        Xutil.Post(ApiUrl.MYOPEN, map, new MyCallBack<MyOpenClassResponse>() {
            @Override
            public void onSuccess(MyOpenClassResponse result) {
...
//                        mRecyclerView.setAdapter(new CommonAdapter<MyOpenClassResponse.mData>(mContext, R.layout.item_my_open_class, result.getData()) {
//
//                            @Override
...
//                        });
    ...
    private CommRecyclerViewAdapter<MyOpenClassResponse.mData> getAdapter() {
        if (null == mAdapter) {
            mAdapter = new CommRecyclerViewAdapter<MyOpenClassResponse.mData>(this, R.layout.item_my_open_class, null) {
                @Override
                protected void convert(ViewHolderZhy holder, final MyOpenClassResponse.mData mData, final int position) {
                    ImageView imageView = (ImageView) holder.getConvertView().findViewById(R.id.open_class_image);
                    Glide.with(mContext).load(mData.getImage()).apply(GloableConstant.getInstance().getDefaultOptionssmall()).into(imageView);
                    holder.setText(R.id.open_class_name, mData.getName());
                    holder.setText(R.id.open_class_content, mData.getDescription());
                    ...
                    if (status.equals("6")) {
                        ...
                        holder.setOnClickListener(R.id.item_open_class, new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ...
                                intent.putExtra("token",mData.getLive_info().getToken());
