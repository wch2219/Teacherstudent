...
/**
 * 当前类注释:
 * 作者：jinwenfeng on 2016/12/6 a1:52
 * 邮箱：823546371@qq.com
 * QQ： 823546371
 * 公司：南京穆尊信息科技有限公司
 * © 2016 jinwenfeng
...
public class PullToRefreshLayoutRewrite extends FrameLayout {
    ...
    private int loading = R.layout.progress_dialog, empty = com.jwenfeng.library.R.layout.layout_empty, error = com.jwenfeng.library.R.layout.layout_error;
    private int noLoadMore = 0;

    public int getNoLoadMore() {
        return noLoadMore;
    }

    public void setNoLoadMore(int noLoadMore) {
    ...
    public void setPullRefreshListener(PullRefreshListener refreshListener) {
        this.pullrefreshListener = refreshListener;
    }

    public void setOnErrorListener(onClickCallBackListener callBackListener) {
        this.onErrorClickListener = callBackListener;
    }
    ...
    public PullToRefreshLayoutRewrite(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PullToRefreshLayoutRewrite(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        TypedArray a = context.obtainStyledAttributes(attrs, com.jwenfeng.library.R.styleable.PullToRefreshLayout, defStyleAttr, 0);
        error = a.getResourceId(com.jwenfeng.library.R.styleable.PullToRefreshLayout_view_error, error);
        loading = a.getResourceId(com.jwenfeng.library.R.styleable.PullToRefreshLayout_view_loading, loading);
        empty = a.getResourceId(com.jwenfeng.library.R.styleable.PullToRefreshLayout_view_empty, empty);
                ...
                if (dy1 > 0 && canRefresh) {
                    if (dy1 >= head_height) {
                        createAnimatorTranslationY(State.REFRESH,
                                ...
                                new com.jwenfeng.library.pulltorefresh.PullToRefreshLayout.CallBack() {
                                    @Override
                    ...
                    if (noLoadMore == 0) {
                        if (canLoadMore) {
                            if (Math.abs(dy1) >= foot_height) {
                                createAnimatorTranslationY(State.LOADMORE, Math.abs(dy1) > foot_height_2 ? foot_height_2 : Math.abs(dy1), foot_height, new com.jwenfeng.library.pulltorefresh.PullToRefreshLayout.CallBack() {
                                    @Override
                                    public void onSuccess() {
                                        isLoadMore = true;
                                        if (refreshListener != null) {
     ...
     * 创建动画
     */
    public void createAnimatorTranslationY(@State.REFRESH_STATE final int state, final int start, final int purpose, final com.jwenfeng.library.pulltorefresh.PullToRefreshLayout.CallBack callBack) {
    ...
    private void setFinish(int height, @State.REFRESH_STATE final int state) {
        createAnimatorTranslationY(state, height, 0, new com.jwenfeng.library.pulltorefresh.PullToRefreshLayout.CallBack() {
            @Override
            public void onSuccess() {
                if (state == State.REFRESH) {
                    isRefresh = false;
                ...
                errorView.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (onErrorClickListener != null) {
                            onErrorClickListener.onCallBackListener(v);
                        }
                    }
                });
                break;
